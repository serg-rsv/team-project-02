...
export async function renderTrendingInfinity() { 
  	const movies = await TmdbApiService.fetchTrendingMovies()			// робимо запит на сервер і отримуємо масив з даними про фільми,які в тренді
...
export async function renderQueryInfinity(query) {
  	const movies = await TmdbApiService.fetchSearchMovie(query); // робимо запит на сервер і отримуємо масив з даними про фіільми,які влаштовують наш пошук
	renderMainPage(movies); 			// викликаємо функцію рендеру сторінки з запиту
